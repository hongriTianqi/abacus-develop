...
TEST_F(ParaGlobal,SplitGrid)
{
	...
	if(nproc==4)
	{
		...
		if(my_rank==0) EXPECT_EQ(mpi.grank,0);
		if(my_rank==1) EXPECT_EQ(mpi.grank,1);
		if(my_rank==2) EXPECT_EQ(mpi.grank,0);
		if(my_rank==3) EXPECT_EQ(mpi.grank,1);
		...
		if(my_rank==0) EXPECT_EQ(mpi.grank,0);
		if(my_rank==1) EXPECT_EQ(mpi.grank,0);
		if(my_rank==2) EXPECT_EQ(mpi.grank,0);
		if(my_rank==3) EXPECT_EQ(mpi.grank,0);
	}
	else
	{
		Parallel_Global::split_grid_world(nproc,
                                    nproc,
                                    my_rank,
                                    mpi.grank,
                                    mpi.gsize);
...
TEST_F(ParaGlobal,SplitDiag)
{
	...
	if(nproc==4)
	{
		...
		if(my_rank==0) EXPECT_EQ(mpi.drank,0);
		if(my_rank==1) EXPECT_EQ(mpi.drank,0);
		if(my_rank==2) EXPECT_EQ(mpi.drank,1);
		if(my_rank==3) EXPECT_EQ(mpi.drank,1);
		...
		if(my_rank==0) EXPECT_EQ(mpi.drank,0);
		if(my_rank==1) EXPECT_EQ(mpi.drank,1);
		if(my_rank==2) EXPECT_EQ(mpi.drank,2);
		if(my_rank==3) EXPECT_EQ(mpi.drank,3);
	}
	else
	{
		Parallel_Global::split_diag_world(nproc,
                                    nproc,
                                    my_rank,
                                    mpi.drank,
                                    mpi.dsize,
...
TEST_F(ParaGlobal, DivideMPIPools)
{
    ...
    mpi.kpar = 3;
    this->my_rank = 5;
    Parallel_Global::divide_mpi_groups(this->nproc, mpi.kpar, this->my_rank, mpi.nproc_in_pool, mpi.my_pool, mpi.rank_in_pool);
